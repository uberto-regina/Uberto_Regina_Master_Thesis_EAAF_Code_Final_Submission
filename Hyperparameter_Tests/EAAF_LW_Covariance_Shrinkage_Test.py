...
"""
Test with Ledoit-Wolf covariance shrinkage estimator from Section 5.3.3 to assess the value of covariance shrinkage.
"""


def compute_maxreturn_capped_vol_LW_Covariance_Shrink(
    data,
    Prices,
    daily_rf_rate,
    ...
    """
    Convex optimization exaclty as compute_maxreturn_capped_vol in EAAF.py, just to emphasize the Ledoit-Wolf Covariance Shrinkage Estimator
    """
    ...
    EW_weights = np.ones(n)/n #np.array([0.6,0.4,0,0,0])
...
def compute_maxreturn_capped_vol_No_Covariance_Shrink(
    data,
    Prices,
    daily_rf_rate,
    previous_weights=None,
    ...
    """
    Convex optimization as compute_maxreturn_capped_vol in EAAF.py, only without Ledoit-Wolf Covariance Shrinkage Estimator
    """
    ...
    #5) Compute historical covariance matrix 
    Sigma = Returns.cov()
    ...
    if gamma > 0:
        shrinkage_term = gamma * cp.sum(u + v)
    else:
        shrinkage_term = 0
...
def expanding_window_training_covariance_shrinkage(MacroEconomic_indicators, Prices, daily_rf_rate):
    ...
    cov_shrinkage = [True, False]

    Weights_test = []


    for j, shrink in enumerate(cov_shrinkage):
        print(f"\n=== TEST RUN {j+1}/{shrink} ===")
        Weights= []
        ...
        for start, end in test_periods:
            ...
            if shrink:
                ...
                Forest = EAAF_parallelized(
                    ...
                    train_macro,
                    train_rf,
                    compute_maxreturn_capped_vol_LW_Covariance_Shrink,
                    max_depth=4,
                ...
                Forest = EAAF_parallelized(
                    ...
                    train_macro,
                    train_rf,
                    compute_maxreturn_capped_vol_No_Covariance_Shrink,
                    max_depth=4,
                    min_samples_split=72,
                    n_trees = 360,
                    bootstrap_frac=1,
...
if __name__ == '__main__':
    ...
    WT = expanding_window_training_covariance_shrinkage(MacroEconomic_indicators, Prices, daily_rf_rate)
    
    with open("EAAF_TEST_COVARIANCE_SHRINKAGE.pkl", "wb") as f:
       pickle.dump(WT, f)
