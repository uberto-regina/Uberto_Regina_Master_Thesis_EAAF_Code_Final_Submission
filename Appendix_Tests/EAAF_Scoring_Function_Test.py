...
def compute_maxreturn_capped_vol_sharpe_ratio_score(
    data,
    Prices,
    daily_rf_rate,
    ...
    """
    Perform the same convex optimization as `compute_maxreturn_capped_vol` in EAAF.py, but return the annualized Sharpe ratio of the optimized portfolio. 

    Specifically, after finding the optimal weights w, compute:
        Sharpe = (μᵀw - r_f) / sigma(w) x √12

    This value is used as the split-score instead of the portfolio's expected return (μᵀwₒₚₜ).
    """
    ...
    sharpe_annual = sharpe * np.sqrt(12.0)

    return sharpe_annual, w_opt

def compute_maxreturn_capped_vol_excess_return_score(
    data,
    Prices,
    daily_rf_rate,
    previous_weights=None,
    ...
    """
    Execute the same convex optimization as `compute_maxreturn_capped_vol` in EAAF.py, 
    just to emphasize the use of the optimal portfolio's expected return (μᵀ wₒₚₜ) as the split score.
    """
...
def expanding_window_training_scoring_test(MacroEconomic_indicators, Prices, daily_rf_rate):
    ...
    scores = ['Excess_Return_Score', 'Sharpe_Ratio_Score']
    ...
    for j, score in enumerate(scores):
        ...
        for start, end in test_periods:
            ...
            if score == "Excess_Return_Score":
                ...
                Forest = EAAF_parallelized(
                    ...
                    train_macro,
                    train_rf,
                    compute_maxreturn_capped_vol_excess_return_score,
                    max_depth=4,
                ...
                Forest = EAAF_parallelized(
                    ...
                    train_macro,
                    train_rf,
                    compute_maxreturn_capped_vol_sharpe_ratio_score,
                    max_depth=4,
                    min_samples_split=72, 
                    n_trees=360,
                    bootstrap_frac=1,
        ...
        Weights_df['Name'] = f"experiment_{j}, score = {score}"
...
if __name__ == '__main__':
    ...
    WT = expanding_window_training_scoring_test(MacroEconomic_indicators, Prices, daily_rf_rate)

    with open("EAAF_TEST_SCORING_FUNCTION.pkl", "wb") as f:
       pickle.dump(WT, f)
